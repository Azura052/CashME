...
if ($dia_actual <= 15) {
    ...
    $fecha_fin = $mes_actual . '-15';
} else {
    $fecha_inicio = $mes_actual . '-16';
    $fecha_fin = date('Y-m-t'); // Último día del mes
}
...
foreach ($headers as $category => $columns) {
    ...
    $query = "SELECT $column_list 
              FROM $category 
              WHERE usuario_idUsuario = ? AND {$columns[2]} BETWEEN ? AND ?
    ...
    $quincena = ($dia_actual <= 15) ? 'primera' : 'segunda';

    // Crear encabezado de la categoría
    $pdf->CategoryHeader($category . 's de la ' . $quincena . ' quincena de ' . strftime('%B', strtotime($mes_actual . '-01')) . ' - ' . $anio_actual, $categories[$category]);
...
// Salida del PDF
$pdf->Output('ResumenQuincenal.pdf', 'I');
?>