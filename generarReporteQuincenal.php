...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Cell(0, 10, 'Resumen de Movimientos Quincenal', 0, 1, 'C');
        ...
        $this->MultiCell(0, 10, 'A continuacion, se presentan los movimientos que registraste durante los ultimos 15 dias. Este informe te ofrece un resumen claro y detallado de las transacciones realizadas y seguimiento de tus actividades financieras.', 0, 'J');
...
$fecha_inicio = date('d/m/Y', strtotime('-15 days')); // Fecha de inicio (hace 15 días)
$fecha_fin = date('d/m/Y'); // Fecha de fin (hoy)

foreach ($categories as $category => $color) {
    $pdf->CategoryHeader($category . 's del ' . $fecha_inicio . ' al ' . $fecha_fin, $color);
    ...
    $query = "SELECT {$category}Desc, {$category}Monto, {$category}Fecha 
              FROM $category 
              WHERE usuario_idUsuario = ? AND {$category}Fecha >= DATE_SUB(CURDATE(), INTERVAL 15 DAY)
